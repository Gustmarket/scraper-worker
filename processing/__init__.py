...
import database
from processing.processors.pre_processed_items import map_and_normalize_pre_processed_items
from processing.processors.raw_items import PreProcessor
...
def upsert_products_from_normalized_items():
    ...
    for normalized_item in normalized_items:
        ...
        products.append({
            ...
            "name": item['name'],
            **(item.get('description') and {"description": item.get('description')} or {}),
            **(item.get('html_description') and {"html_description": item.get('html_description')} or {}),
